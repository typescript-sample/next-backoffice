            ...
            {list && list.length > 0 && list.map((user, i) => {
              return (
                <li key={i} className='col s12 m6 l4 xl3' onClick={e=>edit(e,user.userId)}>
                  <section>
                    <img src={user.imageURL && user.imageURL.length > 0 ? user.imageURL : (user.gender === 'F' ? femaleIcon.src : maleIcon.src)} alt='user' className='round-border' />
                    <div>
                      <h3 className={user.status === 'I' ? 'inactive' : ''}><a>{user.displayName}</a></h3>
                      <p>{user.email}</p>
                    </div>
