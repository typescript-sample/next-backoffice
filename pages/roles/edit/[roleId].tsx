...
import { checkPatternOnBlur, handleError, inputEdit, Status } from '@lib/uione';
...
export function RoleForm() {
  ...
  const handleCheckParent = (e: React.ChangeEvent<HTMLInputElement>, id: string) => {
    ...
    if (parentPrivilege !== undefined) {
      let childrenPermission = parentPrivilege?.children?.map(k => {
  ...
  const handleCheckBox = (event: React.ChangeEvent<HTMLInputElement>, id: string, parentId?: string, currentPrivilege?: Privilege, force?: boolean) => {
    ...
    if (isParent) {
      ...
      let childrenPermission = currentPrivilege?.children?.map(k => {
      ...
      permissions = permissions.map((p) => {
        if (p.id === currentPrivilege.id) {
          ...
          if (pChecked > 1) {
            if (p.permissions < pChecked) { //case Prevent unchecking "read" if "write" is unchecked, but "delete" is not unchecked => read checked
      ...
      if (parentId !== "") {
        ...
        if (privilegeParent !== undefined) {
          let childrenPermission = privilegeParent?.children?.map(k => {
            const e = permissions.find(i => i.id === k.id);
            if (e !== undefined) {
  ...
  const renderForms = (modules: Privilege[] | undefined, parentId: string, disabled: boolean, isChild?: boolean) => {
    ...
    return modules.map((m) =>
        renderForm(m, parentId, disabled, isChild)
    );
  };
            ...
            <div className="row">
              <div className="flex-gap-2 col s4">
                ...
                <input
                    type="checkbox"
                    disabled={disabled}
                    onChange={e => handleCheckParent(e, p.id)}
                    checked={isParentChecked(p.id, p.children, privileges)}
                />
                <p>{p.name}</p>
                {resource.select_all}
              </div>
